...
// ── Init ──────────────────────────────────────────────────────────────────
// Handle OAuth callback — Supabase redirects back with tokens in the URL hash
function _handleOAuthCallback() {
  ...
  if (access_token) {
    _saveSession({ access_token, refresh_token, expires_in: Number(expires_in), token_type });
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const wasOAuth = _handleOAuthCallback();

    showLoading();
    await loadCreators();

    // Render page immediately — don't block on auth
    updateAuthUI();
    handleRoute();
    window.addEventListener('popstate', handleRoute);
    initSearch();

    // Background: auth refresh + non-critical data (don't block page)
    loadFavouriteCounts(); // fire-and-forget
    refreshAuth().then(() => { updateAuthUI(); if (wasOAuth) handleRoute(); }).catch(() => updateAuthUI());
  } catch (e) {
...
function updateNavActive(path) {
  const links = { navHome: '/', navDiscover: '/discover', navRankings: '/rankings', navBecome: '/become-a-creator' };
...
function handleRoute() {
  ...
  try {
    if (path === '/' || path === '/index.html') {
      ...
      // OAuth callback — tokens are parsed in _handleOAuthCallback() before routing
      currentRoute = { page: 'home' };
      renderHome();
...
// Click handler for internal links — respects Ctrl/Cmd+Click to open in new tab
function isModifiedClick(e) { return e.ctrlKey || e.metaKey || e.shiftKey || e.altKey || e.button === 1; }
...
function signInWithGoogle() {
  const siteUrl = window.location.hostname === 'localhost' ? window.location.origin : 'https://fanreactionsfc.com';
        ...
        <div class="sc-body">
          <div class="become-section">
            ...
            <div class="become-text">
              ...
              <a href="/become-a-creator" class="btn-yellow">Read the Full Guide &rarr;</a>
            </div>
          </div>
        </div>
      </div>
...
function battleRenderHot() {
  ...
  const items = battleLeaderboard.map(r => {
    ...
    return `<a class="battle-hot-item" href="${creatorLink(c)}" >
    ...
    <div class="container" style="padding-top:28px;padding-bottom:60px">
      ...
      <div class="tutorial-body">
        ...
        <ul>
          ...
          <li><strong>Submit your channel</strong> — <a href="/submit">add yourself to FanReactionsFC</a> so fans can discover you</li>
        </ul>
        ...
        <div class="tutorial-cta">
          ...
          <a href="/submit" class="btn-yellow">+ Submit Your Channel</a>
        </div>
      </div>
    </div>
    ${renderFooter()}
